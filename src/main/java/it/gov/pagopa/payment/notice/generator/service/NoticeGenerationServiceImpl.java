...
public class NoticeGenerationServiceImpl implements NoticeGenerationService {
	...
	public NoticeGenerationServiceImpl(PaymentGenerationRequestRepository paymentGenerationRequestRepository,
			PaymentGenerationRequestErrorRepository paymentGenerationRequestErrorRepository,
			InstitutionsStorageClient institutionsStorageClient, NoticeStorageClient noticeStorageClient,
			NoticeTemplateStorageClient noticeTemplateStorageClient, PdfEngineClient pdfEngineClient,
			Aes256Utils aes256Utils, ObjectMapper objectMapper, Validator validator,
	...
	/**
	 * Generate a notice, if required content is provided and valid, and saves the
	 * content to a folderId if provided and valid
	 *
	 * @param noticeGenerationRequestItem request data to use for the notice
	 *                                    generation
	...
	@SneakyThrows
	@Override
	public File generateNotice(NoticeGenerationRequestItem noticeGenerationRequestItem, String folderId,
			String errorId) {
		...
		try {
			...
			Path tempDirectory = Files.createTempDirectory(workingDirectory.toPath(), "notice-generator").normalize()
					.toAbsolutePath();

			CreditorInstitution creditorInstitution = institutionsStorageClient
					.getInstitutionData(noticeGenerationRequestItem.getData().getCreditorInstitution().getTaxCode());
			noticeGenerationRequestItem.getData().setCreditorInstitution(creditorInstitution);

			File templateFile = noticeTemplateStorageClient.getTemplate(noticeGenerationRequestItem.getTemplateId());
			...
			String templateData = objectMapper
					.writeValueAsString(TemplateDataMapper.mapTemplate(noticeGenerationRequestItem.getData()));
	...
	/**
	 * Retrieves the notice code, given a {@link NoticeGenerationRequestItem}. If
	 * the notice has a code, it is returned. Otherwise, the code of the first
	 * installment is returned. If no installments are present, a random UUID is
	 * generated.
	 *
	...
	public static String getNoticeCode(NoticeGenerationRequestItem noticeGenerationRequestItem) {
		...
		return Optional.ofNullable(code).orElse(UUID.randomUUID().toString());
	}
	...
	private PdfEngineResponse callPdfEngine(PdfEngineRequest request, Path tempDirectory) {
		...
		if (pdfEngineResponse.getStatusCode() != HttpStatus.SC_OK) {
			String errMsg = String.format("PDF-Engine response KO (%s): %s", pdfEngineResponse.getStatusCode(),
					pdfEngineResponse.getErrorMessage());
			log.error(errMsg);
	 ...
	 */
	private void validateTemplate(NoticeGenerationRequestItem noticeGenerationRequestItem,
			TemplateResource templateResource) throws JsonProcessingException {
		if (templateResource != null && templateResource.getTemplateValidationRules() != null) {
			...
			if (!validationMessageSet.isEmpty()) {
				List<String> value = validationMessageSet.stream().map(ValidationMessage::getMessage).toList();
	...
	private void addNoticeIntoFolder(String itemId, String folderId, PdfEngineResponse pdfEngineResponse) {
		...
		try (BufferedInputStream pdfStream = new BufferedInputStream(
			...
			if (PaymentGenerationRequestStatus.PROCESSING.equals(paymentNoticeGenerationRequest.getStatus())
					&& paymentNoticeGenerationRequest.getNumberOfElementsTotal() <= paymentNoticeGenerationRequest
							.getItems().size() + paymentNoticeGenerationRequest.getNumberOfElementsFailed()
	...
	private void findFolderIfExists(String folderId) {
		PaymentNoticeGenerationRequest ignored = paymentGenerationRequestRepository.findById(folderId)
	...
	private void publishCompletionEventOrRollback(String folderId,
		...
		try {
			 ...
			 * publication.
			 */
			paymentGenerationRequestRepository.findAndSetToProcessing(folderId);

			throw new CompletionEventPublicationException(folderId, e);
		}
